...
export default function TensionCalculator() {
  ...
  return (
    <div>
      ...
      <h1 className="screen-title">{t('tc_title')}</h1>
      <p className="screen-subtitle">{t('tc_subtitle')}</p>
      ...
      <Link to="/tools/wire" style={{ fontSize: 13 }}>{t('tc_wire_link')}</Link>
      ...
      <div className="card" style={{ textAlign: 'center' }}>
        {valid ? (
          ...
          <div style={{ color: 'var(--text-faint)' }}>{t('tc_invalid')}</div>
        )}
      </div>
    </div>
  )
